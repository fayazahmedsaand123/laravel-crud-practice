        ...
        <div class="navbar-right">
            ...
            @if($user && $user->image)
                <img src="{{ asset('Register_Image/'.$user->image) }}"
                    class="image-profile" alt="Register_Image">
            @else
...
@section('content')
    ...
    <div class="main-center">
        <h1><i class="fa fa-pencil-square me-2" aria-hidden="true"></i>Update Country</h1>
        <form action="{{ route('update_country_post',$update_country->id) }}" id="update_country" method="post" enctype="multipart/form-data">
            @csrf
            {{-- Country name field --}}
            <div class="form-group">
                <label class="form-label">Country Name </label>
                <div class="input-wrapper">
                    <input type="text" name="country_name" value="{{ $update_country->country_name }}" placeholder="Enter country">
            ...
            <div class="form-group">
                ...
                <div class="input-wrapper">
                    <input type="text" name="capital_name" value="{{ $update_country->capital_name }}" placeholder="Enter capital">
                    <div class="error-icon">!</div>
                </div>
            </div>
            {{-- Gender field --}}
            <div class="form-group">
                ...
                <div class="input-wrapper">
                    <input type="text" name="gender_name" value="{{ $update_country->gender_name }}" placeholder="Enter gender">
                    <div class="error-icon">!</div>
                </div>
            </div>
            {{-- Color field --}}
            <div class="form-group">
                ...
                <div class="input-wrapper">
                    <input type="text" name="color_name" value="{{ $update_country->color_name }}" placeholder="Enter color">
                    <div class="error-icon">!</div>
                </div>
            </div>
            {{-- Dropdown Student --}}
            <div class="form-group">
                ...
                <select name="student_id" class="select-foregin">
                    ...
                    @foreach($students as $student)
                        <option value="{{ $student->id }}" {{ $update_country->student_id == $student->id ? 'selected' : '' }}>{{ $student->id }}</option>
                    @endforeach
                <select>
            ...
            <div class="form-group">
                ...
                <select name="teacher_id" class="select-foregin">
                    ...
                    @foreach ($teachers as $teacher)
                        <option value="{{ $teacher->id }}" {{ $update_country->teacher_id == $teacher->id ? 'selected' : '' }}>{{ $teacher->id }}</option>
                    @endforeach
                </select>
                <div class="error error-teacher_id"></div>
            </div>
            {{-- Dropdown Subject --}}
            <div class="form-group">
                ...
                <select name="subject_id" class="select-foregin">
                    ...
                    @foreach ($subjects as $subject)
                        <option value="{{ $subject->id }}" {{ $update_country->subject_id == $subject->id ? 'selected' : '' }}>{{ $subject->id }}</option>   
                    @endforeach
                </select>
                <div class="error error-subject_id"></div>
            </div>
            {{-- Image picture --}}
            <div class="form-group">
                <img id="imagePreview" src="{{$update_country->image ? asset('/Country_Image/'.$update_country->image) :
                    asset('/Country_Image/default.png') }}" alt="Country Image">
            </div>
            {{-- Image field --}}
            <div class="form-group">
...
@section('scripts')
    <script>
        $(document).ready(function() {
            $('#update_country').on('submit',function(e) {
                e.preventDefault();
                var formData = new FormData(this);
                var update_country = $(this).attr('action');
