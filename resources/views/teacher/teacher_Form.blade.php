...
@section('content')
    ...
    <div class="main-center">
        <h1>➕ Teacher Form</h1>
        <form action="{{ route('teacher_Form') }}" id="teacher_AjaxPost" method="post" enctype="multipart/form-data">
            @csrf
            {{-- Name field --}}
            ...
            <div class="form-group">
                ...
                <div class="input-wrapper">
                    <input type="text" name="nationally" placeholder="Enter nationally">
                    <div class="error-icon">!</div>
                </div>
            </div>
            {{-- Student Dropdown --}}
...
@section('scripts')
    <script>
        $(document).ready(function() {
            $('#teacher_AjaxPost').on('submit',function(e) {
                e.preventDefault();
                var formData = new FormData(this);
                
                $.ajax({
                    url:"{{ route('teacher_Form') }}",
                    type:'post',
                    data:formData,
                    processData:false,
                    contentType:false,
