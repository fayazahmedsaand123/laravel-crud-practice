...
@section('content')
    ...
    <div class="main-center">
        <h1>➕ Computer Form</h1>
        <form action="{{ route('computer_add') }}" id="ajax_computer" method="post" enctype="multipart/form-data">
            @csrf
            {{-- Name field --}}
            <div class="form-group">
                <label class="form-label">Computer Name</label>
                <div class="input-wrapper">
                    <input type="text" name="computer_name" placeholder="Enter computer">
            ...
            <div class="form-group">
                ...
                <div class="input-wrapper">
                    <input type="number" name="price" placeholder="Enter price number">
                    <div class="error-icon">!</div>
                </div>
            </div>
            {{-- Description field --}}
            ...
            <div class="form-group">
                ...
                <select name="subject_id" class="select-foregin">
                    <option class="select" value="">Select Subjects</option>
            ...
            <div class="form-group">
                ...
                <select name="country_id" class="select-foregin">
                    <option class="select" value="">Select Countries</option>
                    @foreach($countries as $country)
                        <option value="{{ $country->id }}">{{ $country->id }}</option>
                    @endforeach
                </select>
                <div class="error error-country_id"></div>
            </div>
            {{-- Image picture --}}
            <div class="form-group">
...
@section('scripts')
    <script>
        $(document).ready(function() {
            $('#ajax_computer').on('submit',function(e) {
                e.preventDefault();
                var formData = new FormData(this);
                
                $.ajax({
                    url:"{{ route('computer_add') }}",
                    type:'post',
                    data:formData,
                    processData:false,
                    contentType:false,
