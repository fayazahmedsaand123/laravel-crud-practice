...
@section('content')
    <div class="container d-flex justify-content-center mt-4">
        <div class="main-center w-100" style="max-width: 800px;">
            <h3 class="text-center mb-3">🔠 Alphabet</h3>
            <div class="card shadow-lg">
                <div class="card-body">
                    <div class="row g-3"><!-- Grid inside the card -->
                        @foreach($alphabets as $alphabet)
                            <div class="col-6 col-sm-4 col-md-3 col-lg-4">
                                <div class="card position-relative text-center shadow-sm" style="color: {{ $alphabet->color ?? '#1a202c' }}">
                                    <!-- Top X Button -->
                                    <a href="{{ route('delete_alphabet', $alphabet->id) }}" 
                                        data-id="{{ $alphabet->id }}" class="btn btn-danger btn-sm position-absolute top-0 end-0 m-1 px-2 delete-btn">X</a>
                                    <div class="card-body p-2">
                                        <div class="fs-4">{{ $alphabet->emoji ?? '🔤' }}</div>
                                        <div class="fw-bold mt-1">{{ $alphabet->letter }}</div>
                                    </div>
                                </div>
                            </div>
                        @endforeach
    ...
    <script>
        ...
        $(document).ready(function() {
            $('.delete-btn').click(function(e) {
                e.preventDefault();
                var id = $(this).data('id');
                if(!confirm('Are you want to delete this alphabet?')) {
                    return false;
                }

                $.ajax({
                    url:'/delete_alphabet/' + id,
                    type:'get',
                    dataType:'json',
