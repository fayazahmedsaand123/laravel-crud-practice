...
@section('content')
    ...
    <div class="main-center">
        <h1><i class="fa fa-pencil-square me-2" aria-hidden="true"></i>Update Subject</h1>
        <form action="{{ route('update_subject_post',$update_subject->id) }}" id="update_subject" method="post" enctype="multipart/form-data">
            @csrf
            {{-- Name field --}}
            <div class="form-group">
                <label class="form-label">First Name </label>
                <div class="input-wrapper">
                    <input type="text" name="name" value="{{ $update_subject->name }}" placeholder="Enter name">
            ...
            <div class="form-group">
                ...
                <div class="input-wrapper">
                    <input type="text" name="age" value="{{ $update_subject->age }}" placeholder="Enter age">
                    <div class="error-icon">!</div>
                </div>
            </div>
            {{-- Religious field --}}
            <div class="form-group">
                ...
                <div class="input-wrapper">
                    <input type="text" name="religious" value="{{ $update_subject->religious }}" placeholder="Enter religious">
                    <div class="error-icon">!</div>
                </div>
            </div>  
            {{-- Phone number field --}}
            <div class="form-group">
                ...
                <div class="input-wrapper">
                    <input type="text" name="phone" value="{{ $update_subject->phone }}" placeholder="Enter phone">
                    <div class="error-icon">!</div>
                </div>
            </div>
            {{-- Dropdown Student --}}
            <div class="form-group">
                ...
                <select name="student_id" class="select-foregin">
                    <option class="select" value="">Select Students</option>
                    @foreach ($students as $student)
                        <option value="{{ $student->id }}"
                            {{ $update_subject->student_id == $student->id ? 'selected' : '' }}>
                            {{ $student->id }} </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select name="teacher_id" class="select-foregin">
                    <option class="select" value="">Select Teachers</option>
                    @foreach ($teachers as $teacher)
                        <option value="{{ $teacher->id }}" {{ $update_subject->teacher_id == $teacher->id ? 'selected' : '' }}>{{ $teacher->id }}</option>
                    @endforeach
                </select>
            ...
            {{-- Image picture --}}
            <div class="form-group">
                <img id="imagePreview" src="{{$update_subject->image ? asset('/Subject_Image/'.$update_subject->image) :
                    asset('/Subject_Image/default.png') }}" alt="Subject Image">
            </div>
            {{-- Image field --}}
            <div class="form-group">
...
@section('scripts')
    <script>
        $(document).ready(function() {
            $('#update_subject').on('submit',function(e) {
                e.preventDefault();
                var formData = new FormData(this);
                var update_subject = $(this).attr('action');
